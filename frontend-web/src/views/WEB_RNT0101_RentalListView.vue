<!--
  機能ID: WEB_RNT0101
  views/WEB_RNT0101_RentalListView.vue
  -----------------------------------------------
  レンタル一覧画面コンポーネント
  -----------------------------------------------
-->
          ...
          <table class="table">
            ...
            <tbody>
              ...
              <tr v-for="r in rentals" :key="r.id" :class="{ 'row-expired': r.expired, 'row-near': !r.expired && r.daysUntilExpiry !== null && r.daysUntilExpiry <= 90 }">
                ...
                <td>
                  <!-- 詳細ボタン（IT_STAFF のみ表示。ポップアップで詳細表示・編集・返却登録が可能） -->
                  <button
...
/**
 * レンタル契約情報を更新する（ポップアップの「更新」ボタン押下時に呼ばれる）
 * @param req - 更新リクエスト（ベンダー・契約番号・開始日・終了日・月額）
 */
async function updateRental(req: RentalUpdateRequest) {
  if (!detailRental.value) return
  saving.value = true
  try {
    await rentalsApi.update(detailRental.value.id, req)
    showDetailModal.value = false
    loadRentals()
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : (e as { response?: { data?: { message?: string } } })?.response?.data?.message ?? '更新失敗'
    alert(msg)
  } finally {
    saving.value = false
  }
...
async function doReturn(r: PcRental) {
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : (e as { response?: { data?: { message?: string } } })?.response?.data?.message ?? '返却登録失敗'
    alert(msg)
  }
}
